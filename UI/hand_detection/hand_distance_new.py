...
while True:
    ...
    if success:
        ...
        if hands:
            ...
            # Display the resized image on white background
            cv2.imshow("Zoomed Image", whiteImage)

            exec_time = time.time() - start

            if is_thumbs_up(lmHand) and exec_time > 10:
                # If thumbs-up gesture is detected and more than 10 seconds have passed, stop drawing
                drawing = False
                if not done_sending:
                    # Fill the matrix with the coordinates of the drawn points
                    for alive in coordinates:
                        ...
                        if (alive[1] - 1 > -1) and (alive[0] + 1 < 1280):
                            matrix[int((alive[1] - 1) * 1.5)][int((alive[0] + 1) * 2)] = 1
                    try:
                        # Serialize the matrix and send it to the client
            ...
            if drawing:
                # Draw lines between thumb and index finger if distance is large enough
                ...
                if is_hand_open(lmHand) and done_sending:
                    # Send pause message if hand is open and matrix has been sent
                    message = "P"  # Pause
                    print(message)
